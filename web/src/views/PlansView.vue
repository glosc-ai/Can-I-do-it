...
import { CircleAlertIcon, DownloadIcon, FileTextIcon, PlayIcon } from '@lucide/vue'
      ...
      <section class="flex flex-col gap-3" aria-labelledby="plans-list-title">
        ...
        <template v-else>
            ...
            <Table>
              ...
              <TableBody v-else>
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant="secondary">{{ plan.filename === 'idea-analysis.txt' ? '想法' : fileTypeLabel(plan.filename, plan.mime_type) }}</Badge>
                  </TableCell>
          ...
          <Empty v-else class="border">
            ...
            <EmptyContent>
              <Button variant="outline" size="sm" @click="composerRef?.focus()">
                开始第一次分析
              </Button>
            </EmptyContent>
          </Empty>
