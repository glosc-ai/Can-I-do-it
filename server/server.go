...
func run(ctx context.Context) error {
	...
	objectStore := storage.New(db, cfg.Database.Driver, cfg.EncryptionKey, cfg.Storage.Directory, cfg.Storage.MaxUploadBytes, storage.R2Config{
		Enabled:         cfg.Storage.R2.Enabled,
		...
		withSecurityHeaders(
			withCORS(cfg.CORSOrigins,
				withRequestLog(logger,
					withRequestID(withCSRF(userService.Middleware(mux))),
				),
			),
		),
	)
